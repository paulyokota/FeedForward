...
class SyncService:
    ...
    def _strip_feedforward_metadata(self, description: Optional[str]) -> Optional[str]:
        """
        Strip FeedForward metadata block from a Shortcut description.

        When we push to Shortcut, we append a metadata section. When pulling back,
        we need to strip it to avoid duplicating metadata in the local description.

        The metadata block starts with "---\\n## Metadata" and ends with
        "*Generated by FeedForward pipeline*"
        """
        if not description:
            return description

        # Pattern matches the metadata block we add during push
        pattern = r'\n*---\n## Metadata[\s\S]*?\*Generated by FeedForward pipeline\*\s*$'
    ...
    def pull_from_shortcut(self, story_id: UUID) -> SyncResult:
        ...
        try:
            ...
            # Update internal story (strip our metadata block from description)
            clean_description = self._strip_feedforward_metadata(
                shortcut_story.description
            )

...
class TestClassificationGuidedExploration:
    ...
    def test_build_code_context_dict_structure(
        ...
        mock_exploration.success = True
        mock_exploration.error = None
        mock_exploration.relevance_metadata = None  # Optional field
    ...
    def test_build_code_context_dict_includes_relevance_metadata(
        self,
        mock_story_service,
        mock_orphan_service,
    ):
        """Test that relevance_metadata is included in code_context dict (Issue #198)."""
        service = StoryCreationService(mock_story_service, mock_orphan_service)

        # Create mock exploration result with relevance_metadata
        mock_exploration = Mock()
        mock_exploration.relevant_files = []
        mock_exploration.code_snippets = []
        mock_exploration.exploration_duration_ms = 100
        mock_exploration.success = True
        mock_exploration.error = None
        mock_exploration.relevance_metadata = {
            "match_score": 47,
            "matched_terms": ["scheduling", "timeout"],
            "high_signal_matched": ["scheduling"],
        ...
        mock_classification.keywords_matched = ["scheduling"]
        mock_classification.classification_duration_ms = 50

        code_context = service._build_code_context_dict(mock_exploration, mock_classification)

        # Verify relevance_metadata is included
        assert "relevance_metadata" in code_context
        assert code_context["relevance_metadata"] is not None
        assert code_context["relevance_metadata"]["match_score"] == 47
        assert code_context["relevance_metadata"]["high_signal_matched"] == ["scheduling"]
        assert code_context["relevance_metadata"]["threshold_passed"] is True
        assert code_context["relevance_metadata"]["gated"] is False

    def test_build_code_context_dict_relevance_metadata_none(
        self,
        mock_story_service,
        mock_orphan_service,
    ):
        """Test that relevance_metadata can be None (backwards compatibility)."""
        service = StoryCreationService(mock_story_service, mock_orphan_service)
        ...
        mock_exploration.success = True
        mock_exploration.error = None
        mock_exploration.relevance_metadata = None  # Not set (pre-#198 data)

        code_context = service._build_code_context_dict(mock_exploration, None)

        # relevance_metadata key exists but is None
        assert "relevance_metadata" in code_context
        assert code_context["relevance_metadata"] is None
...
class TestMismatchDetection:
    ...
    def test_mismatch_detected_when_categories_differ(self):
        ...
        mock_exploration.success = True
        mock_exploration.error = None
        mock_exploration.relevance_metadata = None  # Issue #198
...
# =============================================================================
# Implementation Context Wiring Tests (Issue #198 - Phase 2)
# =============================================================================


class TestImplementationContextWiring:
    """Tests for implementation_context generation in all 4 story creation paths."""

    @pytest.fixture
    def mock_services(self):
        """Create mock story and orphan services."""
        story_service = Mock()
        ...
        orphan_service = Mock()
        return story_service, orphan_service

    @pytest.fixture
    def sample_conversations(self):
        """Create sample ConversationData for testing."""
        return [
            ConversationData(
                id="conv1",
                issue_signature="scheduling_issue",
                product_area="scheduling",
                component="pin_scheduler",
                symptoms=["TimeoutError when saving pins"],
                user_intent="save pin for later",
            ),
            ConversationData(
                id="conv2",
                issue_signature="scheduling_issue",
                product_area="scheduling",
                component="pin_scheduler",
                symptoms=["Pins don't save"],
                user_intent="schedule a pin",
            ),
            ConversationData(
                id="conv3",
                issue_signature="scheduling_issue",
                product_area="scheduling",
                component="pin_scheduler",
                symptoms=["Error saving"],
                user_intent="save scheduled pin",
            ),
        ]

    def test_create_story_with_evidence_generates_implementation_context(
        self, mock_services, sample_conversations
    ):
        """_create_story_with_evidence should call _generate_implementation_context when enabled."""
        story_service, orphan_service = mock_services

        # Create service with implementation_context enabled via mock service
        with patch.object(
            StoryCreationService, "_generate_implementation_context"
        ) as mock_gen_impl:
            ...
            # Provide implementation_context_service to enable the feature
            mock_impl_service = Mock()
            service = StoryCreationService(
                story_service=story_service,
                orphan_service=orphan_service,
                dual_format_enabled=False,  # Disable to avoid codebase exploration
                implementation_context_service=mock_impl_service,
            )

            result = ProcessingResult()
            service._create_story_with_evidence(
                signature="test_signature",
            ...
            # Verify _generate_implementation_context was called
            assert mock_gen_impl.called

    def test_handle_keep_together_generates_implementation_context(
        self, mock_services, sample_conversations
    ):
        """_handle_keep_together should generate implementation_context when enabled."""
        story_service, orphan_service = mock_services
        ...
        with patch.object(
            ...
            result = ProcessingResult()
            service._handle_keep_together(pm_result, sample_conversations, result)

            # Verify implementation context generation was attempted
            assert mock_gen_impl.called

    def test_create_story_from_subgroup_generates_implementation_context(
        self, mock_services, sample_conversations
    ):
        """_create_story_from_subgroup should generate implementation_context when enabled."""
        story_service, orphan_service = mock_services
        ...
        with patch.object(
            ...
            result = ProcessingResult()
            service._create_story_from_subgroup(
    ...
    def test_create_story_from_hybrid_cluster_generates_implementation_context(
        self, mock_services, sample_conversations
    ):
        """_create_story_from_hybrid_cluster should generate implementation_context when enabled."""
        story_service, orphan_service = mock_services
        ...
        with patch.object(
            ...
            # Create mock cluster with all required attributes
            mock_cluster = Mock()
            mock_cluster.embedding_cluster = 1
            mock_cluster.action_type = "request"
            mock_cluster.direction = "add"
            mock_cluster.cluster_id = "emb_1_facet_request_add"  # Required string field

            result = ProcessingResult()
            service._create_story_from_hybrid_cluster(
    ...
    def test_implementation_context_passed_to_story_create(
        self, mock_services, sample_conversations
    ):
        """implementation_context should be passed to StoryCreate when generated."""
        story_service, orphan_service = mock_services

        expected_impl_context = {
            "summary": "Implementation guidance here",
            "relevant_files": [{"path": "src/scheduler.py", "rationale": "Main logic"}],
            "next_steps": ["Update scheduler logic"],
            "source": "hybrid",
            "success": True,
        }
        ...
        with patch.object(
            ...
            mock_gen_impl.return_value = expected_impl_context
            ...
            # Verify StoryCreate was called with implementation_context
            story_service.create.assert_called_once()
            call_args = story_service.create.call_args
            story_create = call_args[0][0]  # First positional arg
            assert story_create.implementation_context == expected_impl_context

    def test_implementation_context_disabled_skips_generation(
        self, mock_services, sample_conversations
    ):
        """When implementation_context service not provided, should not generate context."""
        story_service, orphan_service = mock_services

        with patch.object(
            StoryCreationService, "_generate_implementation_context"
        ) as mock_gen_impl:
            # Don't provide implementation_context_service to disable the feature
            service = StoryCreationService(
                story_service=story_service,
                orphan_service=orphan_service,
                dual_format_enabled=False,
                # implementation_context_service not provided
            )
            ...
            # Should not call _generate_implementation_context
            mock_gen_impl.assert_not_called()


# =============================================================================
# Relevance Metadata in Score Metadata Tests (Issue #198 - Phase 4)
# =============================================================================


class TestRelevanceMetadataInScoreMetadata:
    """Tests for relevance_metadata being plumbed to score_metadata."""
    ...
    def test_relevance_metadata_injected_into_score_metadata(self, sample_conversations):
        """relevance_metadata should be injected into scores.metadata as relevance_metrics."""
        from src.multi_factor_scorer import MultiFactorScorer, MultiFactorScores

        # Create mock scorer
        mock_scorer = Mock(spec=MultiFactorScorer)
        mock_scorer.score.return_value = MultiFactorScores(
        ...
        # Provide multi_factor_scorer to enable the feature
        service = StoryCreationService(
            story_service=Mock(),
            orphan_service=Mock(),
            multi_factor_scorer=mock_scorer,
        )
        ...
        scores = service._compute_multi_factor_scores(
            conversations=sample_conversations,
            relevance_metadata=relevance_metadata,
        )

        # Verify relevance_metrics was added
        assert scores is not None
        assert "relevance_metrics" in scores.metadata
        rm = scores.metadata["relevance_metrics"]
        assert rm["match_score"] == 47
        assert rm["matched_terms"] == ["scheduling", "timeout"]
        assert rm["high_signal_matched"] == ["scheduling"]
        assert rm["term_diversity"] == 2
        assert rm["threshold_passed"] is True
        assert rm["gated"] is False

        # Verify existing metadata preserved
        assert scores.metadata["existing_key"] == "existing_value"

    def test_relevance_metadata_not_injected_when_none(self, sample_conversations):
        """When relevance_metadata is None, should not add relevance_metrics."""
        from src.multi_factor_scorer import MultiFactorScorer, MultiFactorScores
        ...
        scores = service._compute_multi_factor_scores(
            conversations=sample_conversations,
            relevance_metadata=None,  # Not provided
        )

        assert scores is not None
        assert "relevance_metrics" not in scores.metadata
        assert scores.metadata["existing_key"] == "existing_value"

    def test_relevance_metadata_preserves_gated_flag(self, sample_conversations):
        """When gated=True in relevance_metadata, should be preserved in score_metadata."""
        from src.multi_factor_scorer import MultiFactorScorer, MultiFactorScores
        ...
        relevance_metadata = {
            ...
            "high_signal_matched": [],
            "term_diversity": 1,
            "threshold_passed": False,  # Failed threshold
            "gated": True,  # Was gated
        }
        ...
        assert scores is not None
        rm = scores.metadata["relevance_metrics"]
        assert rm["threshold_passed"] is False
        assert rm["gated"] is True

    def test_relevance_metadata_no_key_collision(self, sample_conversations):
        """Should not overwrite existing relevance_metrics key if already present."""
        from src.multi_factor_scorer import MultiFactorScorer, MultiFactorScores

        mock_scorer = Mock(spec=MultiFactorScorer)
        # Pre-existing relevance_metrics in metadata (shouldn't happen, but test safety)
        mock_scorer.score.return_value = MultiFactorScores(
            actionability_score=50.0,
            fix_size_score=60.0,
            severity_score=40.0,
            churn_risk_score=30.0,
            metadata={"relevance_metrics": {"pre_existing": True}},
        )
        ...
        scores = service._compute_multi_factor_scores(
            conversations=sample_conversations,
            relevance_metadata=new_relevance_metadata,
        )

        # Should NOT overwrite existing key
        assert scores.metadata["relevance_metrics"] == {"pre_existing": True}

    def test_relevance_metadata_extracted_from_code_context(self):
        """Verify relevance_metadata is extracted from code_context in story creation paths."""
        story_service = Mock()
        story_service.create.return_value = Story(
            id=uuid4(),
            title="Test",
        ...
        code_context_with_relevance = {
            "relevant_files": [{"path": "test.py"}],
            "relevance_metadata": {
                "match_score": 50,
                "matched_terms": ["scheduling"],
                "high_signal_matched": ["scheduling"],
                "term_diversity": 1,
                "threshold_passed": True,
                "gated": False,
        ...
        with patch.object(
            StoryCreationService, "_explore_codebase_with_classification"
        ) as mock_explore:
            mock_explore.return_value = code_context_with_relevance

            with patch.object(
                StoryCreationService, "_compute_multi_factor_scores"
            ) as mock_compute:
                ...
                # Verify _compute_multi_factor_scores received relevance_metadata
                mock_compute.assert_called_once()
                call_kwargs = mock_compute.call_args[1]
                assert "relevance_metadata" in call_kwargs
                assert call_kwargs["relevance_metadata"]["match_score"] == 50
                assert call_kwargs["relevance_metadata"]["threshold_passed"] is True

    def test_multi_factor_scoring_disabled_returns_none(self, sample_conversations):
        """When multi_factor_scoring_enabled=False, should return None."""
        import os

        # Disable multi-factor scoring via environment variable
        with patch.dict(os.environ, {"MULTI_FACTOR_SCORING_ENABLED": "false"}):
            service = StoryCreationService(
                story_service=Mock(),
                orphan_service=Mock(),
            )
            ...
            scores = service._compute_multi_factor_scores(
                conversations=sample_conversations,
                relevance_metadata={"match_score": 100},
            )

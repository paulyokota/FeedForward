...
PROMPT_FILE="${PROMPT_FILE:-ralph_v2/PROMPT.md}"
MAX_ITERATIONS="${MAX_ITERATIONS:-40}"
COMPLETION_PROMISE="${COMPLETION_PROMISE:-COMPLETE}"
TIMEOUT_MINUTES="${TIMEOUT_MINUTES:-30}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-ralph_v2/checkpoints}"
PROGRESS_FILE="${PROGRESS_FILE:-ralph_v2/progress.txt}"
REBASE_STATE_FILE="${REBASE_STATE_FILE:-ralph_v2/checkpoints/rebase_fail.count}"
CRITERIA_FILE="${CRITERIA_FILE:-ralph_v2/criteria.json}"
...
for ((i=1; i<=MAX_ITERATIONS; i++)); do
  echo "=== Ralph V2 iteration $i/$MAX_ITERATIONS ==="

  OUTPUT_FILE="ralph_v2/ralph-out-$i.log"
  ...
  if grep -qi "awaiting test run approval" "$OUTPUT_FILE"; then
    echo "❌ Ralph V2 output indicates tests were not run. Stopping iteration."
    exit 1
  fi
  ...
  if [ -f "$PROGRESS_FILE" ] && tail -n 20 "$PROGRESS_FILE" | rg -qi "blocked"; then
    echo "❌ Ralph V2 output indicates BLOCKED state in $PROGRESS_FILE. Stopping iteration."
    exit 1
  fi

  if grep -q "$COMPLETION_PROMISE" "$OUTPUT_FILE"; then
    if [ ! -f "$CRITERIA_FILE" ]; then
      echo "❌ Completion promise found but $CRITERIA_FILE is missing."
      exit 1
    fi
    if rg -q "\"[^\"]+\"\\s*:\\s*false" "$CRITERIA_FILE"; then
      echo "❌ Completion promise found but criteria are not all true in $CRITERIA_FILE."
      exit 1
    fi
